...
import { createRuleSystem, listRuleSystems } from '@/server/rule-system-service';
import { conflictError, internalServerError, successResponse, validationError } from '@/server/response';
import { ZodError } from 'zod';

export async function GET() {
  try {
    return successResponse(await listRuleSystems());
  } catch {
    return internalServerError('Failed to list rule systems');
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    return successResponse(await createRuleSystem(body), 201);
  } catch (error) {
    ...
    return internalServerError('Failed to create rule system');
  }
}
