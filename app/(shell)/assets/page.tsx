        ...
        <div className="list-stack">
          {items.map((item) => (
            <article key={item.id} className="list-item">
              <p className="list-item__type">{ui.common.entityTypes.asset}</p>
              <div className="list-item__topline">
                <div>
                  <h2 className="list-item__title"><Link href={`/assets/${item.id}`}>{item.name}</Link></h2>
                  <p className="list-item__subtitle">{item.fileName} В· {item.mimeType} В· {ui.assets.assetKindOptions[item.assetKind]}</p>
                </div>
